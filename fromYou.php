...
<head>
	...
	<link rel="stylesheet" href="/stylesheets/fromYou.css">
	<script>
	$(function(){
    ...
    <nav class="drawer-nav" role="navigation">
      <ul class="drawer-menu">
        ...
        <li>
			<div id="good">
				<p>いいね</p>
				<button type="button" id="fromPersonbtn" class="cButton">相手からのいいね！</button>
  ...
  <main role="main">
	<div id="main">
		<div class="contents" id="fromYou">
			<p class="main-title">Follower</p>
			<%
			var cnt=0;
			while(true){
				if(userList.length == cnt ||userList.length == 0){
					break;
				}
			%>
			<div class="fromYouUser">
				...
				<div class="buttonWrapper">
					<button type="button" id="f<%= userList[cnt].user_id %>" class="fButton" data-user_id=<%= userList[cnt].user_id %>>FollowBack</button>
					<a href="/message"><button type="button" id="m<%= userList[cnt].user_id %>" class="mButton" data-user_id=<%= userList[cnt].user_id %>>メッセージに移動</button></a>
				</div>
			</div>
			<%
				cnt++;
			...
			%>
				<div class="empty-contents">
					<p class="empty-contents-title">誰からもフォローされていません。</p>
				</div>
			<%}%>
		</div>
	</div>
  ...
  <!-- ドロワーメニューの利用宣言 -->
	<script>
		...
		$('.fButton').on('click',function(){
			...
			socket.emit("remove", {sender : $(this).data('user_id'), receiver : '<%= user.user_id %>'});
			socket.emit("matching", {sender : $(this).data('user_id'), receiver : '<%= user.user_id %>'});
			$('#f' + $(this).data('user_id')).css('display','none');
			
			//$('#fbutton').css('display','none');
			$('#m' + $(this).data('user_id')).stop(true, true).fadeIn('500');
		});	
	</script>
</body>
</html>
